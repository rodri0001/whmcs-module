...
");
        ...
        return [
            'status' => 'success',
            'description' => 'Modul kurulumu gerçekleşmiştir. Lütfen "Eklentiler > rodri Domain" adımlarını takip ederek gerekli ayarları yapın. ',
        ];
    } catch (\Exception $e) {
        return [
            'status' => "error",
...
function tr_domain_deactivate()
{
    try {
        Capsule::schema()
            ->dropIfExists('mod_rodridomain');
        Capsule::schema()
            ->dropIfExists('mod_rodridomainlog');
        return [
            'status' => 'success',
...
require_once ("modules/registrars/rodri/rodri.php");
    ...
    if($_REQUEST['uploaddomain']){
       ...
       $data =  rodri_SendMultiDocument($uploaddata);
       if(!empty($data['error'])){
           $status = "error";
       } else {
           $status = "success";
    ...
    foreach($domainresults['domains']['domain'] as $domaindata){
        ...
        if(($domaindata['registrar'] == 'rodri') and ($domaindata['status'] == 'Pending Registration')){
            $domainstatus = Capsule::table('mod_rodridomain')->where('domainname', '=', $domaindata['domainname'])->first();
