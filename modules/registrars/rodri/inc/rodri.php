...
function rodri_getRequest($command, $postfields, $auth){
    ...
    $sTarget = "https://api.rodri.com/v2/".$command;
    $ch = curl_init();
...
function rodri_CheckDomainStatus($params){
...
function rodri_DomainInfo($params){
    $params = array_merge($params, rodri_GetConfigurationParamsData());
    $auth['ApiSecret'] = $params['ApiSecret'];
    $auth['ApiKey'] = $params['ApiKey'];
    $postfields = array();
...
function rodri_GetConfigurationParamsrData() {
...
function rodri_GetConfigurationParamsData() {
    $result = Capsule::table('tblregistrars')->where('registrar', 'rodri')->get();
    foreach($result as $item) {
        $params[$item->setting] = decrypt($item->value);
    }
    return $params;
}
function rodri_GetDomainData($domain) {
    $domain = Capsule::table('tbldomains')->where('domain', $domain)->first();
    $return = json_decode(json_encode($domain), true);
    return $return;
}
function rodri_GetDomainUserData($domain) {
...
function rodri_ErrorMesages($params){
...
function rodri_ActionTypes($params){
...
function rodri_addDomainLog($array){

    $result = Capsule::table('mod_rodridomain')->where('domainname', $array['domainname'])->first();
    ...
    logModuleCall('rodri', 'addDomainLog', $array, $_FILES['attachments']['name'], '', '');
    $array['actioncomment'] = trim(str_replace('[Lutfen Seciniz]', '', $array['actioncomment']));
    if((!$result->id)){
        $insert = Capsule::table('mod_rodridomain')->insertGetId(
            [
        ...
        $result = Capsule::table('mod_rodridomainlog')->insert(
            [
                'domainid' => $insert,
                'userid' => $array['userid'],
                'ticketid' => $array['ticketid'],
        ...
        if($array['logtype'] == 'action'){
            ...
            $insert = Capsule::table('mod_rodridomainlog')->insert(
                [
                    'domainid' => $result->id,
            ...
            if (!empty($array['ticketid'])) {
                if (($result->lastdescription != $array['lastdescription']) or ($result->actiontype != $array['actiontype'])) {
                    $update = Capsule::table('mod_rodridomain')->where('id', $result->id)->update(
                        [
                            'ticketid' => $array['ticketid'],
                            'laststatus' => $array['laststatus'],
                            'actiontype' => $array['actiontype'],
...
function rodri_ViewDomainLogs($params){
    $domain = $params['sld'].'.'.$params['tld'];
    //$params = array_merge($params,rodri_GetConfigurationParamsData());
    $domaindata = Capsule::table('mod_rodridomain')->where('domainname', $domain)->first();

    $result = Capsule::table('mod_rodridomainlog')->where('domainid', $domaindata->id)->orderby('id', 'asc')->get();
    $return = json_decode(json_encode($result), true);
    $table = '<!--Table-->
    ...
    foreach($return as $notesData) {
        $action = "";
        $action = rodri_ActionTypes($notesData['actiontype']). ' - ';
...
function rodri_ViewUploadForm($params){
    $domain = $params['sld'].'.'.$params['tld'];
    $params = array_merge($params,rodri_GetConfigurationParamsData());
    $token = generate_token($type = "link");
...
function rodri_ViewClientUploadForm($params){
    $domain = $params['sld'].'.'.$params['tld'];
    $params = array_merge($params,rodri_GetConfigurationParamsData());
...
function rodri_ViewClientMultiUploadForm($params){
...
function rodri_SendMultiDocument($params){
    ...
    $result = $res['result'];
    $logdocuments = "";
    logModuleCall('rodri', 'file1', $params, $_FILES['attachments']['name'], '', '');
